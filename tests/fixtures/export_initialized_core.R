...
writeLines(cdr3exp,file.path(output_dir,'cell_clone_ids.txt'))
writeLines(colnames(exp_data),file.path(output_dir,'selected_barcodes.txt'))
write.csv(initialized$meta_dedup,file.path(output_dir,'annotation.csv'))
...
writeBin(as.double(initialized$master_dist_e),file.path(output_dir,'master_dist_e.bin'),size=8)
writeBin(as.double(as.matrix(t)),file.path(output_dir,'embedding.bin'),size=8)
writeLines(
  c(as.character(nrow(exp_data)),as.character(ncol(exp_data))),
  file.path(output_dir,'expression_shape.txt')
)
writeBin(as.double(exp_data),file.path(output_dir,'expression.bin'),size=8)
